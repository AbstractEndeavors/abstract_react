...
def path_to_url(path,base_image_url=None,base_image_dir=None):
    if path and base_image_url and base_image_dir and path.startswith(base_image_dir):
        rel_path = path[len(base_image_dir):]
        if rel_path and rel_path.startswith('/'):
            rel_path = rel_path[1:]
        if base_image_url.endswith('/'):
            base_image_url = base_image_url[:-1]
        path = f"{base_image_url}/{rel_path}"
    return path
def get_init_title_variant(title_variants):
    title_variants = title_variants or []
    if title_variants:
        return title_variants[0]
def get_meta_info(info=None,app=False,player_url=False,feed_url=False,geo=False,languages=False,base_image_url=None,base_image_dir=None,**kwargs):
    info = info
    
    path=info.get('path')
    domain = info.get("domain")
    title = info.get('title')
    site_name = get_site_name(info)
    app_name = get_app_name(info)
    domain_name = get_domain_name(info)
    ...
    full_url = any_domain or get_init_title_variant(title_variants)
    share_url = info.get("share_url",full_url) or ""
    canonical = full_url
    ...
    href = info.get('href', '')
    keywords_str = info.get("keywords_str") or ""
    thumbnail = info.get("thumbnail", "")
    thumbnail_link = info.get("thumbnail_link",  path_to_url(thumbnail,base_image_url=base_image_url,base_image_dir=base_image_dir))
    ...
    if thumbnail and not thumbnail_url_resized:  # Only resize if not already provided
        ...
        if img:
            if img.size != (1200, 627):
                ...
                thumbnail_url_resized =  path_to_url(thumbnail_resized,base_image_url=base_image_url,base_image_dir=base_image_dir)
            else:
                thumbnail_resized =  path_to_url(thumbnail,base_image_url=base_image_url,base_image_dir=base_image_dir)
                thumbnail_url_resized =  path_to_url(thumbnail_link,base_image_url=base_image_url,base_image_dir=base_image_dir)
        else:
            # Fallback if validation fails
            thumbnail_resized =  path_to_url(thumbnail_resized,base_image_url=base_image_url,base_image_dir=base_image_dir)
            thumbnail_url_resized =  path_to_url(thumbnail_resized,base_image_url=base_image_url,base_image_dir=base_image_dir)
    
    # Update info with resized values
    info['thumbnail_resized'] = path_to_url(thumbnail_resized,base_image_url=base_image_url,base_image_dir=base_image_dir)
    info['thumbnail_url_resized'] = path_to_url(thumbnail_url_resized,base_image_url=base_image_url,base_image_dir=base_image_dir)
    
    mobile_url = share_url.replace("https://", "https://m.")
    oembed_url = f"{domain}/oembed?url={share_url}"
    actual_image = path_to_url(thumbnail_url_resized or thumbnail_link,base_image_url=base_image_url,base_image_dir=base_image_dir)
    actual_path = path_to_url(thumbnail_resized or thumbnail,base_image_url=base_image_url,base_image_dir=base_image_dir)
    ...
    metadata = {
        ...
        "thumbnail": path_to_url(thumbnail,base_image_url=base_image_url,base_image_dir=base_image_dir),
        "thumbnail_link": path_to_url(thumbnail_link,base_image_url=base_image_url,base_image_dir=base_image_dir),
        "thumbnail_resized": path_to_url(thumbnail_resized,base_image_url=base_image_url,base_image_dir=base_image_dir),
        "thumbnail_url_resized": path_to_url(thumbnail_url_resized,base_image_url=base_image_url,base_image_dir=base_image_dir),
        "canonical": share_url,
        ...
        "og": {
            ...
            "image":  path_to_url(actual_image,base_image_url=base_image_url,base_image_dir=base_image_dir),
            ...
            "article": {
                ...
                "section": info.get("section") or "General",
                "tag": [t.strip() for t in keywords_str.split(",") if t.strip()]
            }
        },
        "twitter": {
